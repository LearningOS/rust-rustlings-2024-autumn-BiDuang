    ...
    T: Default,
{
    ...
    fn next(&mut self) -> Option<T> {
        ...
        let ret = self.items.swap_remove(self.count);
        self.count -= 1;

        let mut i = 1;
        let mut s = self.smallest_child_idx(i);
        while s <= self.count && ((self.comparator)(&self.items[s], &self.items[i])) {
            self.items.swap(s, i);
            i = s;
            s = self.smallest_child_idx(s);
        }
